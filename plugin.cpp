...
StatusCode g_HandleCreateObj(unsigned char* p_pUUID, ObjectRef* p_ppObj) {
    ...
    if (memcmp(p_pUUID, H265AMFEncoder::encoderInfo.UUID, 15) == 0) {
        const uint8_t formatIndex = p_pUUID[15] - H265AMFEncoder::encoderInfo.UUID[15];
        *p_ppObj = new H265AMFEncoder(formatIndex);
        return errNone;
    }
...
StatusCode g_ListCodecs(HostListRef* p_pList) {
    ...
    err = H265AMFEncoder::RegisterCodecs(p_pList);
    if (err != errNone) return err;
...
StatusCode g_GetEncoderSettings(unsigned char* p_pUUID, HostPropertyCollectionRef* p_pValues,
    ...
    if (memcmp(p_pUUID, H265AMFEncoder::encoderInfo.UUID, 15) == 0) {
        return H265AMFEncoder::GetEncoderSettings(p_pValues, p_pSettingsList);
    }
